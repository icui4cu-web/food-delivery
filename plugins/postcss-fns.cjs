module.exports = () => ({
	postcssPlugin: 'postcss-px',
	Declaration(decl) {
		if (
			!decl.value.includes('fluid(') &&
			!decl.value.includes('em(') &&
			!decl.value.includes('cqw(')
		) return;
		...
		decl.value = decl.value.replace(
			...
			(_, args) => {
				const parts = args.split(',').map(s => s.trim());
				const px = parseFloat(parts[0]);
				const base = parts[1] ? parseFloat(parts[1]) : 1240;
				return `${parseFloat((px * 100 / base).toFixed(5))}cqw`;
			}
		);
	}
});
